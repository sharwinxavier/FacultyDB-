				...
				if(mysqli_num_rows($sql) == 0){
					...
					$age1 = date_diff(date_create($DOB), date_create('now'))->y;
						...
						<tr>
                            <td>'.$no.'</td>
							<td>'.$row['ID'].'</td>	
							<td><a style="color:#322d65;font-weight:bold;" href="profile.php?ID='.$row['ID'].'"><span class="glyphicon glyphicon-user" aria-hidden="true" style="color:#322d65;"></span> '.$row['name'].'</a></td>						
                            <td>'.$row['Gender'].'</td>
                            <td>'.$age1.'</td>
                            <td>'.$row['Position'].'</td>
							<td>'.$row['Mailid'].'</td>
