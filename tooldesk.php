...
<div id="Menualign"></div>
			<div id="navbar" class="navbar-collapse collapse">
				<ul class="nav navbar-nav" style="float: right;">
					...
					<?php
				// 	if($dept=='ADMIN'){
				// 	echo'<li class="navhover" style="color: white; font-weight: bold;"><a href="tools.php">
	...
	<div class="container"><br>
			<h2 class="categories__title">Admin Tools </h2>
			<hr />
			...
			<table class="table table-hover">
				...
				<?php
				if($filter){
					$sql = mysqli_query($con, "SELECT * FROM users WHERE Department='$filter' ORDER BY ID ASC");
				}else{
					$sql = mysqli_query($con, "SELECT * FROM users ORDER BY ID ASC");
				}
				...
				// 	$age1 = date_diff(date_create($DOB), date_create('now'))->y;
						...
						if($row['Department']!="ADMIN" and $dept=="ADMIN"){
						    echo'<a href="edittools.php?id='.$row['id'].'" title="Edit Data" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
							<a href="tooldesk.php?action=delete&id='.$row['id'].'" title="Delete Data" onclick="return confirm(\'Are you sure to delete '.$row['username'].'?\')" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>';
							}else{}
						echo'</td>';
						echo'</tr>';
						$no++;
